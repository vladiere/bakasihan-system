<template>
  ...
  >
    <template v-slot:top>
      <span class="text-h6 text-weight-bold">Today's Sale</span>
      <q-space />
      <q-input dense outlined rounded color="accent" v-model="filter" label="Search..." />
    </template>
  </q-table>
</template>
...
<script setup lang="ts">
  ...
  const pagination = ref({ rowsPerPage: 6 });
