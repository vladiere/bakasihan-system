...
export const createUser = async(req:Request,res:Response)=>{
    try {
        ...
        const {username,password,first_name,last_name,gender}:userRegT = req.body;
        ...
        const response = await executeQuery(insertQuery,[username,hashedPassword]) as QueryResponse;
        if(response && response.insertId){
            await executeQuery(insertUserInfoQuery,[response.insertId,first_name,last_name,gender])
...
export const loginUser = async (req: Request, res: Response) => {
    try {
        const CheckingQuery = 'SELECT id, username, password, status,role FROM user_tbl WHERE username = ?';
        const UpdateQuery = "UPDATE user_tbl SET status = 'online' WHERE id = ?"
        ...
        await executeQuery(UpdateQuery,[user.id]).then(async()=>{
...
export const Token = async(req:Request,res:Response)=>{
    try {
        ...
        const CheckingQuery = 'SELECT id, username, password, status,role FROM user_tbl WHERE id = ?';
...
export const logout = async(req:Request,res:Response)=>{
    try {
        ...
        const updateQuery = "UPDATE user_tbl SET status = 'offline' WHERE id = ?"
        const result = await executeQuery(DeleteQuery,[userAuth.id]);
        if(!result){
            return res.status(404).send({message:'User Not Found'})
        }
        await executeQuery(updateQuery,[userAuth.id]).then(()=>{
